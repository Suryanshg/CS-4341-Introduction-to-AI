...
def General_Search(problem, searchMethod):
    ...
    explored=[] # explored = Make-Empty-Set
    ...
    while len(queue) > 0:  
        ...
        explored.append(node.nodes[0]) # explored = node added to explored
        opened_nodes = Expand(problem, node) # Get new nodes to add to the queue based on the expanded node.
        # queue= opened-nodes added to queue according to search-method only if not in the queue already (with the same or lower value) or in explored
        pathsToRemove=[]
        removed=[]
        for path in opened_nodes: 
            alreadyInExplored=False
            alreadyInQueue=False
            for node in explored: # Search for the node in explored
                if(path.nodes[0].name==node.name):
            ...
            for existingPath in queue: # Search if it exists already in queue (with same or lower value)
                if((path.nodes[0].name==existingPath.nodes[0].name) and (searchMethod in informedSearchAlgos)): # If exists, then search if it exists with same or lower value
